  ...
  return (
    <div className="flex justify-between items-center p-4 px-10 absolute w-full bg-gradient-to-b from-[#1e2126] to-transparent">
      ...
      <div className="md:hidden ">
        <h2
          className="px-3 pb-2 py-1 text-white font-medium flex items-center bg-gray-700 rounded-md cursor-pointer gap-3"
        ...
        {toggle ? (
          <ul className="absolute bg-gray-700 w-[200px] text-center mt-3 left-0 right-0 ml-auto mr-auto rounded-md px-3">
            {menu.map((item) => (
              <li className="text-gray-400 text-[18px] font-medium cursor-pointer mb-3 mt-2 hover:bg-gray-600 hover:text-white px-3 pb-2 py-1 rounded-md transition-all duration-500 ease-in-out">
                {item.name}
              </li>
            ))}
          </ul>
